...
def request_url( type, id=None):
    ...
    if type == "club":
        ...
        url = "web/carto/StructureDetail.asp?"
        options = urllib.urlencode({"stcode":id, "annee":2017, "struct":"ligue"})
...
def write_json( output, headers, elts):
    with open(output, "ab") as jsonfile:
        json.dump(elts, jsonfile)

def write_stdout( output, headers, elts):
    print headers
    print elts
...
def ffv_vacuum(dpt_list=range(0,100), output_format="csv", regattas_file=None, clubs_file=None, details=False):
    try:
        ...
        if regattas_file is not None or output_format =='stdout':
            ...
            write(regattas_file, infos, planning)
...
def main():
    ...
    parser.add_option("-f", "--format", type="string", default="stdout")
